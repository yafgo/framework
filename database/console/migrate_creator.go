...
func (receiver MigrateCreator) Create(name string, table string, create bool) {
	...
	file.Create(receiver.getPath(name, "up"), []byte(receiver.populateStub(upStub, table)))

	//Create the down.sql file.
	file.Create(receiver.getPath(name, "down"), []byte(receiver.populateStub(downStub, table)))
}
