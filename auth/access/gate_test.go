...
import (
	...
	"github.com/yafgo/framework/contracts/auth/access"
)
...
func (s *GateTestSuite) TestWithContext() {
	ctx := context.WithValue(context.Background(), "hello", "yafgo")
	...
	assert.Equal(s.T(), access.NewDenyResponse("yafgo"), gate.Inspect("create", map[string]any{
		"user": "2",
	}))
}
