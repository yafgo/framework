...
import (
	...
	httpvalidate "github.com/yafgo/framework/contracts/validation"
	...
	tests := []Case{
		{
			...
			setup: func(c Case) {
				validator, err := validation.Make(map[string]any{
					...
					"name":  "required",
					"name1": "required_if:name,yafgo,yafgo1",
				})
				assert.Nil(t, err, c.description)
				assert.NotNil(t, validator, c.description)
				assert.False(t, validator.Fails(), c.description)
		...
		},
		{
			...
			setup: func(c Case) {
				...
				assert.NotNil(t, validator, c.description)
				assert.Equal(t, map[string]string{
					"required_if": "name1 is required when name is [yafgo,yafgo1]",
				}, validator.Errors().Get("name1"))
			},
		},
		{
		...
		},
		{
			...
			setup: func(c Case) {
				validator, err := validation.Make(map[string]any{
					...
					"name":  "required",
					"name1": "required_unless:name,yafgo,yafgo1",
				})
				assert.Nil(t, err, c.description)
				assert.NotNil(t, validator, c.description)
				assert.False(t, validator.Fails(), c.description)
	...
	tests := []Case{
		{
			...
			setup: func(c Case) {
				validator, err := validation.Make(map[string]any{
					"name":  "https://www.yafgo.dev",
					"name1": "http://www.yafgo.dev",
				}, map[string]string{
