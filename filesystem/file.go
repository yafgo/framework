...
import (
	...
	supportfile "github.com/yafgo/framework/support/file"
...
func (f *File) GetClientOriginalExtension() string {
	return supportfile.Extension(f.filename)
}
...
func (f *File) HashName(path ...string) string {
	...
	extension := supportfile.Extension(f.file)
	if extension == "" {
...
func (f *File) Extension() (string, error) {
	return supportfile.Extension(f.file), nil
}
