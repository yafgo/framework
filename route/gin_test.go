...
import (
	...
	mockconfig "github.com/yafgo/framework/contracts/config/mocks"
	...
	tests := []struct {
	...
	}{
		{
			...
			setup: func(method, url string) error {
				...
				req.Header.Set("Hello", "yafgo")

				return nil
			},
			expectCode: http.StatusOK,
			expectBody: "{\"full_url\":\"\",\"header\":\"yafgo\",\"id\":\"1\",\"ip\":\"\",\"method\":\"GET\",\"name\":\"Yafgo\",\"path\":\"/get/1\",\"url\":\"\"}",
		},
		{
			name:   "Headers",
			method: "GET",
		...
		},
		{
			...
			setup: func(method, url string) error {
				...
				if err := writer.WriteField("name", "Yafgo"); err != nil {
					return err
				}
		...
		},
		{
			name:   "QueryArray",
			method: "GET",
			url:    "/query-array?name=Yafgo&name=Yafgo1",
			setup: func(method, url string) error {
			...
			expectCode: http.StatusOK,
			expectBody: "{\"name\":[\"Yafgo\",\"Yafgo1\"]}",
		},
		{
			name:   "QueryMap",
			method: "GET",
			url:    "/query-array?name[a]=Yafgo&name[b]=Yafgo1",
			setup: func(method, url string) error {
				gin.Get("/query-array", func(ctx httpcontract.Context) {
					ctx.Response().Success().Json(httpcontract.Json{
			...
			expectCode: http.StatusOK,
			expectBody: "{\"name\":{\"a\":\"Yafgo\",\"b\":\"Yafgo1\"}}",
		},
		{
			name:   "File",
			method: "POST",
		...
		},
		{
			...
			method: "GET",
			url:    "/validator/validate/success?name=Yafgo",
			setup: func(method, url string) error {
		...
		},
		{
			...
			method: "GET",
			url:    "/validator/validate/fail?name=Yafgo",
			setup: func(method, url string) error {
		...
		},
		{
			...
			method: "GET",
			url:    "/validator/validate-request/success?name=Yafgo",
			setup: func(method, url string) error {
			...
			expectCode: http.StatusOK,
			expectBody: "{\"name\":\"Yafgo1\"}",
		},
		{
			name:   "GET with validator but validate request fail",
			method: "GET",
			url:    "/validator/validate-request/fail?name1=Yafgo",
			setup: func(method, url string) error {
		...
		},
		{
			...
			setup: func(method, url string) error {
				gin.Get("/string", func(ctx httpcontract.Context) {
					ctx.Response().String(http.StatusCreated, "Yafgo")
				})
		...
		},
		{
			...
			setup: func(method, url string) error {
				gin.Get("/success/string", func(ctx httpcontract.Context) {
					ctx.Response().Success().String("Yafgo")
				})
		...
		},
		{
			...
			setup: func(method, url string) error {
				gin.Get("/header", func(ctx httpcontract.Context) {
					ctx.Response().Header("Hello", "yafgo").String(http.StatusOK, "Yafgo")
				})
		...
		},
		{
			...
			setup: func(method, url string) error {
				gin.GlobalMiddleware(func(ctx httpcontract.Context) {
					ctx.Response().Header("global", "yafgo")
					ctx.Request().Next()

					assert.Equal(t, "Yafgo", ctx.Response().Origin().Body().String())
					assert.Equal(t, "yafgo", ctx.Response().Origin().Header().Get("global"))
				...
				gin.Get("/origin", func(ctx httpcontract.Context) {
					ctx.Response().String(http.StatusOK, "Yafgo")
				})
