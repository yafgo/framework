	...
	tests := []struct {
	...
	}{
		{
			...
			setup: func() {
				...
				assert.Equal(t, &TestResponse{Code: http.StatusOK, Message: "Yafgo: server: yafgo-server, client: yafgo-client"}, res)
				assert.Nil(t, err)
			},
		},
		{
...
func serverInterceptor(ctx context.Context, req any, info *grpc.UnaryServerInfo, handler grpc.UnaryHandler) (resp any, err error) {
	...
	ctx = context.WithValue(ctx, "server", "yafgo-server")
	if len(md["client"]) > 0 {
...
func (r *TestController) Get(ctx context.Context, req *TestRequest) (*TestResponse, error) {
	if req.GetName() == "success" {
		return &TestResponse{
			Code:    http.StatusOK,
			Message: fmt.Sprintf("Yafgo: server: %s, client: %s", ctx.Value("server"), ctx.Value("client")),
		}, nil
	} else {
